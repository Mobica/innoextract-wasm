...
#else
	return uint16_t((uint16_t(uint8_t(value)) << 8)
	                       | uint8_t(value >> 8));
#endif
}
...
#else
	return (uint32_t(byteswap(uint16_t(value))) << 16)
...
#else
	return (uint64_t(byteswap(uint32_t(value))) << 32)
	       | byteswap(uint32_t(value >> 32));
#endif
}
